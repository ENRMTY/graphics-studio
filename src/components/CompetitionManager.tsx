...
export function CompetitionManager({ competitions, onUpdate }: Props) {
  ...
  const openAdd = () => {
    setEditId(null); setNewName(''); setNewIcon(null); setNewColor('#C8102E'); setAddOpen(true);
  };

  const openEdit = (c: Competition) => {
    setEditId(c.id); setNewName(c.name); setNewIcon(c.icon); setNewColor(c.color); setAddOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (!name) return;
    if (editId) {
      onUpdate(competitions.map((c) => c.id === editId ? { ...c, name, icon: newIcon, color: newColor } : c));
    } else {
      onUpdate([...competitions, { id: Date.now().toString(), name, icon: newIcon, color: newColor }]);
    }
    ...
    <div className="team-manager">
      <div className="team-manager-header">
        <div>
          ...
          <p className="help-text" style={{ marginTop: 2 }}>{competitions.length} competitions</p>
        </div>
      ...
      <div className="comp-manager-grid">
        {competitions.map((c) => (
          <div className="comp-manager-card" key={c.id}>
            <div className="comp-manager-accent" style={{ background: c.color }} />
            <div className="comp-manager-icon">
              {c.icon
                ? <img src={c.icon} alt={c.name} />
                : <Icons.Comps style={{ width: 22, height: 22, color: c.color }} />
              }
            </div>
            <div className="comp-manager-name">{c.name}</div>
            <div className="comp-manager-color">
              <span style={{ width: 10, height: 10, borderRadius: '50%', background: c.color, display: 'inline-block', marginRight: 5 }} />
              {c.color}
            </div>
            <div className="team-card-actions">
              ...
              <button className="btn btn-icon danger" onClick={() => onUpdate(competitions.filter((x) => x.id !== c.id))}>
          ...
          <div className="modal">
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                {newIcon && (
                  <img src={newIcon} alt="" style={{ width: 40, height: 40, objectFit: 'contain', background: 'var(--surface3)', borderRadius: 8, border: '1px solid var(--border)', padding: 4 }} />
                )}
                <button className="btn btn-ghost btn-sm" onClick={iconUpload.open}>
                  <Icons.Upload style={{ width: 12, height: 12 }} />
                  {newIcon ? 'Replace' : 'Upload Icon'}
                </button>
                {newIcon && <button className="btn btn-ghost btn-sm" onClick={() => setNewIcon(null)}>Remove</button>}
              </div>
              <p className="help-text" style={{ marginTop: 6 }}>PNG with transparency works best. Will appear on the graphic next to the competition name.</p>
            </div>
                ...
                {PRESET_COLORS.map((col) => (
                  <button
                    ...
                    style={{
                      ...
                      border: newColor === col ? '3px solid white' : '2px solid transparent',
