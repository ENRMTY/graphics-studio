...
export async function renderFullTime(
  ...
  // — background —
  layer.add(new Konva.Rect({ x: 0, y: 0, width: SIZE, height: SIZE, fill: '#111114' }));
  ...
  const CONTENT_H = BOTTOM_PAD + EVENTS_BLOCK_H + TEAM_NAMES_H + SCORE_H + COMP_H + FT_LABEL_H + TOP_BLOCK_PAD;

  // Gradient overlay — always covers the content area plus a fade above it
  const OVERLAY_H = Math.min(CONTENT_H + 100, SIZE * 0.85);
  layer.add(new Konva.Rect({
    x: 0, y: SIZE - OVERLAY_H,
    width: SIZE, height: OVERLAY_H,
    fillLinearGradientStartPoint: { x: 0, y: 0 },
    fillLinearGradientEndPoint: { x: 0, y: OVERLAY_H },
    fillLinearGradientColorStops: [0, 'rgba(0,0,0,0)', 0.28, 'rgba(0,0,0,0.72)', 1, 'rgba(0,0,0,0.97)'],
  }));

  // — Red bar at very bottom —
  layer.add(new Konva.Rect({ x: 0, y: SIZE - RED_BAR_H, width: SIZE, height: RED_BAR_H, fill: '#C8102E' }));
  ...
  if (maxEvents > 0) {
    ...
    drawEventColumns(layer, homeEvents, awayEvents, cursorY, SIZE, EVENT_LINE_H);
  ...
  layer.add(new Konva.Text({
    ...
    x: 0, y: teamNamesY,
    width: SIZE / 2 - 16, align: 'right',
    fontSize: 18, fontFamily: FONT_DISPLAY, fill: 'rgba(255,255,255,0.65)',
    letterSpacing: 2,
  }));
  layer.add(new Konva.Text({
    text: (data.awayTeam?.name || 'AWAY').toUpperCase(),
    x: SIZE / 2 + 16, y: teamNamesY,
    width: SIZE / 2 - 16, align: 'left',
    fontSize: 18, fontFamily: FONT_DISPLAY, fill: 'rgba(255,255,255,0.65)',
    letterSpacing: 2,
  }));
  ...
  layer.add(new Konva.Text({
    ...
    fontSize: 104, fontFamily: FONT_DISPLAY, fill: '#FFFFFF',
    letterSpacing: 2,
  }));
  ...
  await drawCircularLogo(layer, data.homeTeam?.logo ?? null, LOGO_CX_HOME, LOGO_CY, LOGO_SIZE);
  await drawCircularLogo(layer, data.awayTeam?.logo ?? null, LOGO_CX_AWAY, LOGO_CY, LOGO_SIZE);
  ...
  if (data.competition) {
    ...
    layer.add(new Konva.Rect({
      x: PAD, y: cursorY + 8,
      width: 4, height: 20,
      fill: compColor, cornerRadius: 2,
    }));
    if (data.competitionIcon) {
      try {
        const ci = await loadImage(data.competitionIcon);
        layer.add(new Konva.Image({ image: ci, x: PAD + 12, y: cursorY + 6, width: 24, height: 24 }));
        layer.add(new Konva.Text({
          text: data.competition.toUpperCase(),
          x: PAD + 42, y: cursorY + 10,
          fontSize: 14, fontFamily: FONT_BODY, fill: 'rgba(255,255,255,0.5)',
          letterSpacing: 2, fontStyle: '600',
        }));
      } catch {
        layer.add(new Konva.Text({
          text: data.competition.toUpperCase(),
          x: PAD + 12, y: cursorY + 10,
          fontSize: 14, fontFamily: FONT_BODY, fill: 'rgba(255,255,255,0.5)',
          letterSpacing: 2, fontStyle: '600',
        }));
      }
  ...
  layer.add(new Konva.Text({
    text: 'FULL TIME',
    x: PAD, y: cursorY,
    fontSize: 24, fontFamily: FONT_DISPLAY, fill: '#C8102E', letterSpacing: 5,
  }));
  ...
  layer.add(new Konva.Text({
    text: 'LFC PAGE',
    x: SIZE - PAD, y: 28,
    fontSize: 12, fontFamily: FONT_DISPLAY, fill: 'rgba(255,255,255,0.12)',
    letterSpacing: 3, align: 'right',
  }));
...
export async function renderMatchday(
  ...
  layer.add(new Konva.Rect({
    ...
    fillLinearGradientStartPoint: { x: 0, y: 0 },
    fillLinearGradientEndPoint: { x: 0, y: OVERLAY_H },
    fillLinearGradientColorStops: [0, 'rgba(0,0,0,0)', 0.3, 'rgba(0,0,0,0.78)', 1, 'rgba(0,0,0,0.97)'],
  }));
  ...
  if (data.venue) {
    ...
    layer.add(new Konva.Text({
      ...
      x: PAD, y: cursorY,
      fontSize: 14, fontFamily: FONT_BODY, fill: 'rgba(255,255,255,0.45)',
      letterSpacing: 1, fontStyle: '500',
    }));
  }
  ...
  if (data.matchDate || data.kickoffTime) {
    ...
    if (data.matchDate) {
      ...
      parts.push(d.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }));
    }
    if (data.kickoffTime) parts.push(`KO: ${data.kickoffTime}`);
    layer.add(new Konva.Text({
      text: parts.join('   •   ').toUpperCase(),
      x: PAD, y: cursorY,
      fontSize: 15, fontFamily: FONT_BODY, fill: '#F6C326',
      letterSpacing: 1, fontStyle: '600',
    }));
  }
  ...
  layer.add(new Konva.Text({
    ...
    x: 0, y: cursorY,
    width: SIZE / 2 - 16, align: 'right',
    fontSize: 18, fontFamily: FONT_DISPLAY, fill: 'rgba(255,255,255,0.65)', letterSpacing: 2,
  }));
  layer.add(new Konva.Text({
    text: (data.awayTeam?.name || 'AWAY').toUpperCase(),
    x: SIZE / 2 + 16, y: cursorY,
    width: SIZE / 2 - 16, align: 'left',
    fontSize: 18, fontFamily: FONT_DISPLAY, fill: 'rgba(255,255,255,0.65)', letterSpacing: 2,
  }));
  ...
  layer.add(new Konva.Text({
    ...
    x: 0, y: logoRowCY - 50,
    width: SIZE, align: 'center',
    fontSize: 88, fontFamily: FONT_DISPLAY, fill: 'rgba(255,255,255,0.07)', letterSpacing: 8,
  }));

  await drawCircularLogo(layer, data.homeTeam?.logo ?? null, LOGO_CX_HOME, logoRowCY, LOGO_SIZE);
  await drawCircularLogo(layer, data.awayTeam?.logo ?? null, LOGO_CX_AWAY, logoRowCY, LOGO_SIZE);
  ...
  if (data.competition) {
    ...
    const compColor = data.competitionColor || '#ffffff';
    layer.add(new Konva.Rect({
      x: PAD, y: cursorY + 8, width: 4, height: 20,
      fill: compColor, cornerRadius: 2,
    }));
    if (data.competitionIcon) {
      try {
...
async function drawCircularLogo(
  ...
  try {
    ...
    const group = new Konva.Group({ clip: { x: cx - r, y: cy - r, width: size, height: size } });
    group.add(new Konva.Image({ image: img, x: cx - r, y: cy - r, width: size, height: size }));
    layer.add(group);
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    if (homeEv) {
      ...
      layer.add(new Konva.Text({
        text: txt,
        x: 0, y,
        width: SIZE / 2 - 20, align: 'right',
        fontSize: 18, fontFamily: FONT_BODY,
        fill: 'rgba(255,255,255,0.82)', fontStyle: '500',
      }));
    }
    if (awayEv) {
