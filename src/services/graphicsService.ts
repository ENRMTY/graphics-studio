...
interface SingleResponse { success: boolean; data: ApiGraphic }
interface ListResponse   { success: boolean; total: number; data: ApiGraphic[] }
...
export function apiGraphicToFT(g: ApiGraphic): FullTimeData & { _id?: string } {
  return {
    ...
    homeTeam: g.homeTeamId
      ? { id: g.homeTeamId, name: g.homeTeamName ?? '', logo: g.homeTeamLogoUrl }
      : null,
    awayTeam: g.awayTeamId
      ? { id: g.awayTeamId, name: g.awayTeamName ?? '', logo: g.awayTeamLogoUrl }
      : null,
...
function ftToPayload(data: FullTimeData) {
  return {
    ...
    competitionId:      null,               // we store by name snapshot, not FK lookup
...
export const graphicsService = {
  ...
  async saveFTDraft(data: FullTimeData, id?: string): Promise<ApiGraphic> {
    if (id) {
      const res = await api.patch<SingleResponse>(`/api/graphics/${id}`, ftToPayload(data));
      return res.data;
    }
    const res = await api.post<SingleResponse>('/api/graphics', ftToPayload(data));
    return res.data;
  },
  ...
  async uploadFTBackground(id: string, file: File): Promise<ApiGraphic> {
    ...
    const res = await api.patchForm<SingleResponse>(`/api/graphics/${id}/background`, form);
    return res.data;
  },

  async publishFT(id: string): Promise<ApiGraphic> {
    const res = await api.patch<SingleResponse>(`/api/graphics/${id}`, { status: 'published' });
    return res.data;
  },
  ...
  async saveMDDraft(data: MatchdayData, id?: string): Promise<ApiGraphic> {
    if (id) {
      const res = await api.patch<SingleResponse>(`/api/graphics/${id}`, mdToPayload(data));
      return res.data;
    }
    const res = await api.post<SingleResponse>('/api/graphics', mdToPayload(data));
    return res.data;
  },
